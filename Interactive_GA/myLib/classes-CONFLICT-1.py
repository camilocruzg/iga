...
###Face
class Face():
    def __init__(self,pos,g_size,radius):
        self.r = radius
        self.s = g_size
        self.xy = self.s[0] * self.s[1] * (self.s[2]+1)
        self.xz = self.s[0] * (self.s[1]+1) * self.s[2]
        self.yz = (self.s[0]+1) * self.s[1] * self.s[2]
        self.p = pos
        def getCoord():
            if pos < self.xy:
                x = ((self.p % self.s[0])-(self.s[0]/2)) * self.r
                y = (((self.p // self.s[0]) % self.s[1])-(self.s[1]/2)) * self.r
                z = ((self.p // (self.s[0] * self.s[1]))-(self.s[2]/2)) * self.r
            elif self.xy - 1 < pos < (self.xy + self.xz):
                pos2 = self.p - self.xy
                x = ((pos2 % self.s[0])-(self.s[0]/2)) * self.r
                y = ((pos2 // (self.s[0] * self.s[2]))-(self.s[1]/2)) * self.r
                z = (((pos2 // self.s[0]) % self.s[2])-(self.s[2]/2)) * self.r
            elif (self.xy + self.xz) - 1 < pos < (self.xy + self.xz + self.yz):
                pos3 = self.p - (self.xy + self.xz)
                x = ((pos3 // (self.s[1] * self.s[2]))-(self.s[0]/2)) * self.r
                y = ((pos3 % self.s[1])-(self.s[1]/2)) * self.r
                z = (((pos3 // self.s[1]) % self.s[2])-(self.s[2]/2))  * self.r
            return (x,y,z)
        self.c = getCoord()
        def pts():
            points = []
            if self.p in range(self.xy):
                points.append(self.c)
                points.append(tuple(sum(x) for x in zip(points[0],(self.r,0,0))))
                points.append(tuple(sum(x) for x in zip(points[1],(0,self.r,0))))
                points.append(tuple(sum(x) for x in zip(points[2],(-1*self.r,0,0))))
                return points
            elif self.p in range(self.xy,(self.xy+self.xz)):
                points.append(self.c)
                points.append(tuple(sum(x) for x in zip(points[0],(self.r,0,0))))
                points.append(tuple(sum(x) for x in zip(points[1],(0,0,self.r))))
                points.append(tuple(sum(x) for x in zip(points[2],(-1*self.r,0,0))))
                return points
            else:
                points.append(self.c)
                points.append(tuple(sum(x) for x in zip(points[0],(0,self.r,0))))
                points.append(tuple(sum(x) for x in zip(points[1],(0,0,self.r))))
                points.append(tuple(sum(x) for x in zip(points[2],(0,-1*self.r,0))))
                return points
        self.points = pts()
